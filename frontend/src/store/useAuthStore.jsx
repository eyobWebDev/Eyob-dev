...
export const useAuthStore = create((set, get) => ({
    ...
    saveClientData: async (data) => {
        set({isSavingClientData: true})
        try{
            const res = await Axios.post(`api/auth/client`, data)
            successToaster("Succesfully sent message", "you will soon get detailed response to your personal email", "x")
            console.log(res.data);
            
            set({clientData: res.data})
        }catch (e){
            errorToaster(e.response.data.message)
        } finally {
            set({isSavingClientData: false})
        }
    },

    getClientData: async () => {
        try{
            const res = await Axios.get(`api/auth/get-client-info`)
            console.log(res.data);
            set({clientData: res.data})
        }catch (e){
            errorToaster(e.response.data.message)
